...
tables = [
    measure.regionprops_table(image, properties=properties)
...
images = [
    data.binary_blobs(length=128, n_dim=3, volume_fraction=f)
    for f in fractions
]
...
fig, ax = plt.subplots()
sns.stripplot(
    x='volume fraction', y='area', data=blob_volumes, jitter=True, ax=ax
)
